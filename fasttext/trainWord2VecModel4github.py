        ...
        for fname in os.listdir(self.dirname):
            ...
            for ffname in os.listdir(os.path.join(self.dirname, fname)):
                if '.DS_Store' in ffname:
                    continue
                if "txt" not in ffname:
                    continue
                print (fname,ffname)
                for line in open(os.path.join(self.dirname, fname, ffname)):
                    line = line.split()
...
def submitJobs (path2TextFiles , file2savePath, modelName2save, doBigram, bigramPath, minCountOfWord, dimensionOfVec, workernum):
	...
	model = gensim.models.FastText(sentences,min_count=minCountOfWord,size=dimensionOfVec,max_vocab_size=120000000,workers=workernum,window=5)
